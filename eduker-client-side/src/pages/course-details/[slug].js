...
const CourseDetails = () => {
  ...
  const [request, setRequest] = useState(false);
   const {data:course,isError,isLoading,error } = useGetCourseQuery(id, {
      skip: !request
   })
  //sidebar show
  ...
  // query payment info
  useEffect(() => {
    if (course?._id && user?.email) {
      fetch(`https://eduker-server.onrender.com/paymentInfo?email=${user?.email}&&id=${course?._id}`)
        .then(res => res.json())
        .then(data => {
  ...
  if(!isLoading && !isError && course?._id){
   ...
   const { teacher_img, tutor_name, img_bg, date, title, category, review, price, enrolled,
        lectures, duration } = course || {}
     content = <section className="course__area pt-115 pb-25">
  ...
  return (
    ...
    <SEO pageTitle="Course Details" />
      ...
      <EnrolledModal show={show} handleClose={handleClose} singleCourse={course} />
    </>
  );
};
